...
/** Single source for validation + emitted SQL (Supabase/Express/hybrid). */
const ECHELON_CONTROL_PLANE_SQL_MIGRATION = `-- Control plane tables migration
...
CREATE TABLE IF NOT EXISTS echelon_idempotency (
    tenant_id UUID NOT NULL,
...
CREATE TABLE IF NOT EXISTS echelon_rate_counters (
    api_key_id UUID NOT NULL,
    action TEXT NOT NULL,
    window_bucket TEXT NOT NULL,
    count INT NOT NULL DEFAULT 0,
    updated_at TIMESTAMPTZ DEFAULT NOW(),
    PRIMARY KEY (api_key_id, action, window_bucket)
);

CREATE TABLE IF NOT EXISTS echelon_team_members (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id UUID NOT NULL,
    email TEXT NOT NULL,
    role TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_echelon_team_tenant ON echelon_team_members(tenant_id);

CREATE TABLE IF NOT EXISTS echelon_webhooks (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id UUID NOT NULL,
    url TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_echelon_webhooks_tenant ON echelon_webhooks(tenant_id);

CREATE TABLE IF NOT EXISTS echelon_tenant_settings (
...
function getSqlMigrationTemplate(): string {
  return ECHELON_CONTROL_PLANE_SQL_MIGRATION;
}
...
async function generateSqlMigrations(outputDir: string): Promise<string[]> {
  ...
  const migrationSql = ECHELON_CONTROL_PLANE_SQL_MIGRATION;
